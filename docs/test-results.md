...
- `permit ip 192.168.10.0 0.0.0.255 any` のhit count増加を確認
...
- VLAN99からのSSH: Permit
- VLAN10 / VLAN30からのSSH: Deny

| No. | Source | Destination | Expected | Result |
|---:|---|---|---|---|
| 15 | ADMIN01 (VLAN99) | CORE01 | SSH Success | OK |
| 16 | PC01 (VLAN10) | CORE01 | SSH Deny | OK |
| 17 | DEV01 (VLAN30) | CORE01 | SSH Deny | OK |
...
- `permit 192.168.99.0 0.0.0.255` のmatch count増加を確認
- match countはログイン回数そのものではなく、ACL条件への一致回数として確認
...
以下のようなスクリーンショットを `evidence/` 配下に保存する想定です。
...
GitHubへ公開する際は、個人情報・認証情報・実環境情報が写り込んでいないことを確認します。
